...
export class WoffReader {
  ...
  readTableData(tableEntry: TableEntry): Uint8Array {
    console.log('Reading table data:', tableEntry);
    const { offset, compLength, origLength } = tableEntry;
    let data: Uint8Array;
  
    this.reader.seek(offset);
  
    if (compLength === origLength) {
      console.log('Table is uncompressed');
      data = new Uint8Array(compLength);
      for (let i = 0; i < compLength; i++) {
        data[i] = this.reader.readByte();
      }
    } else {
      console.log('Table is compressed');
      const compressedData = new Uint8Array(compLength);
      for (let i = 0; i < compLength; i++) {
        compressedData[i] = this.reader.readByte();
      }
      data = this.decompressData(compressedData, origLength);
    }
  
    console.log('Table data read, length:', data.length);
    return data;
  }
  
  private decompressData(compressedData: Uint8Array, originalLength: number): Uint8Array {
